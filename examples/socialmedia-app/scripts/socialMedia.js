...
$(document).ready(function () {
    ...
    window.addEventListener('scala', function () {
        var uuid = window.exp.app.config.feed_configuration.uuid;
        var url = '/api/connectors/feeds/' + uuid + '/data';
        var rows = parseFloat(window.exp.app.config.numberRows) || Math.round(4 * window.innerWidth / 1920);
        var width = (1 / rows) * 100;
        var gutter = parseFloat(window.exp.app.config.gutterSize) || 4;
        ...
        var refresh = function () {
            ...
            window.exp.api.get(url, null).then(function (data) {
        ...
        setInterval(GridRebuild, window.exp.app.config.refresh_rate_seconds * 1000);

(function () {
  window.addEventListener('scala', function () {
    //document.getElementById('image').setAttribute('src', scala.app.config.src);
  ...
  canvas = this.__canvas = new fabric.Canvas('c', {
    ...
    selection: false
      ...
      canvas.on('mouse:move', function(options) {
        ...
        scala.iface.trigger({name: 'test', payload: mouse_pos});
      });
  
  scala.iface.when({name: 'test', callback: function (payload) {
    mouse_pos = payload;
  }});
